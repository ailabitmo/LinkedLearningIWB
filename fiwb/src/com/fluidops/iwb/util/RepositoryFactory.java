...
public class RepositoryFactory
{
	...
	public static Repository getRepository(String repositoryType) throws Exception
	{
		...
		if (repositoryType.equals("httprepository"))
			return new HttpRepositoryFactory().loadRepository();
	...
	/**
	 * Return the repository used for historical data. The name is of the
	 * repository is given by {@link Config#getHistoryRepositoryName()} and 
	 * defaults to historymodel. Note that the following native store 
	 * indices are created: "cspo,cpso,cops"
	 * 
	 * @return
	 */
	public static Repository getHistoryRepository() {

		return new NativeRepositoryFactory(Config.getConfig()
				.getHistoryRepositoryName(), "cspo,cpso,cops").loadRepository();
	}
	
	
	/**
	 * Return a repository that stores the data in memory that can
	 * persist the data in the location provided as {@code persistenceFile}.
	 */
	public static Repository getMemoryRepositoryWithPersistence(File persistenceFile) {
		return new MemoryRepositoryFactory(persistenceFile).loadRepository();
	}
	
	/**
	 * Return a repository that stores the data in memory and that does not
	 * perform I/O (not even for persistence, which actually cannot happen).
	 */
	public static Repository getMemoryRepositoryWithoutPersistence() {
		return new MemoryRepositoryFactory(null).loadRepository();
	}
   ...
   /**
	 ...
	 * {@link Config#getNativeStoreIndices()} => the indices used if not specified otherwise
	 * {@link Config#getNativeStoreForceSync()} => specifies whether forced index sync on disk should be used (false by default)
	 * 
	 * @author as
	 */
	private static class NativeRepositoryFactory extends RepositoryFactoryBase {
		...
		private final String nativeStoreIndices;
		...
		public NativeRepositoryFactory(String repositoryName) {
			this(repositoryName, Config.getConfig().getNativeStoreIndices());
		}
		
		public NativeRepositoryFactory(String repositoryName, String nativeStoreIndices) {
			this.repositoryName = repositoryName;
			this.nativeStoreIndices = nativeStoreIndices;
		}

		@Override
		public Repository loadRepository()
		{
			NativeStore nativeStore = ReadDataManagerImpl.getNativeStore(IWBFileUtil.getFileInDataFolder(repositoryName), nativeStoreIndices);
			
			nativeStore.setForceSync(Config.getConfig().getNativeStoreForceSync());
	...
	private static class MemoryRepositoryFactory extends RepositoryFactoryBase {
		...
		public MemoryRepositoryFactory() {
			this(IWBFileUtil.getFileInDataFolder(Config
							.getConfig().getRepositoryName()));
		}

		public MemoryRepositoryFactory(File repositoryFile) {
		...
		public Repository loadRepository()
		{
			...
			if (repositoryFile != null) {
				/*
				 * The returned memory store will allow persistence on the specified repository file.
				 */
				memoryStore = new MemoryStore(repositoryFile);
			} else {
				/*
				 * The returned memory store will not allow persistence.
				 */
				memoryStore = new MemoryStore();
				memoryStore.setPersist(false);
			}

			return new SailRepository(memoryStore);
		}		
	}
...
//			System.setProperty("ftsLiteralsOnly", "false");
	...
	/**
	 * Repository factory for {@link HTTPRepository} instances
	 * 
	...
	private static class HttpRepositoryFactory extends RepositoryFactoryBase {
		...
		public HttpRepositoryFactory(String repositoryServer,
				String repositoryName) {
			this.repositoryServer = repositoryServer;
			this.repositoryName = repositoryName;
		}
