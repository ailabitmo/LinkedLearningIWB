...
public class Config extends DelegateConfig
{
	...
	protected static final boolean PIVOT_ACTIVE_DEFAULT = true;
	...
	@ConfigDoc( name="historyRepositoryName", 
            desc="Directory name of the history repository to store historical data. Default: historymodel",
            category=Category.NONE,
            iwbCategory=IWBCategory.DATABASE,
            type = Type.STRING )
    public String getHistoryRepositoryName()
    { 
        return delegate().get( "historyRepositoryName", "historymodel");
    }
    ...
    @ConfigDoc( name="linkLiterals", 
            desc="If set to true, literals are clickable in Table View. Default: false",
            category=Category.NONE,
            iwbCategory=IWBCategory.CORE,
            type = Type.BOOLEAN)
    public boolean getLinkLiterals() 
    {
    	return delegate().getBoolean("linkLiterals", false);
    }
    ...
    @ConfigDoc( name="defaultDatePattern", 
			desc="The datetime pattern used for rendering, e.g. in the DATETIME value resolver. Default MMM dd, yyyy HH:mm:ss", 
			category=Category.INT,
			iwbCategory=IWBCategory.APPEARANCE,
			type = Type.STRING )
    public String getDefaultDatePattern()
    { 
        return delegate().get( "defaultDatePattern", "MMM dd, yyyy HH:mm:ss" );
    }
    ...
    public Integer[] getChartColorLow()
    {
        ...
        return delegate().getIntArray("chartColorLow", new Integer[] {200,230,255}, 3); 
    ...
    public Integer[] getChartColorHigh()
    {
        ...
        return delegate().getIntArray("chartColorHigh", new Integer[] {10,60,255}, 3); 
	...
	@ConfigDoc( name="labelProperties", 
            desc="Comma separated list of all properties to be treated as label. Values are split by comma (,)",
            category=Category.NONE,
            iwbCategory=IWBCategory.CORE,
    ...
    /**
     * Configures whether forced synchronisation of indices should be enabled for the Sesame native triple store.
     * 
     * @return true if synchronisation is forced, false otherwise.
     */
    @ConfigDoc(name = "nativeStoreForceSync",
            desc = "Configures whether forced synchronisation of indices should be enabled for the Sesame native triple store. Default: false",
            category = Category.NONE,
            iwbCategory = IWBCategory.DATABASE,
            type = Type.BOOLEAN)
    public boolean getNativeStoreForceSync()
    {
        return delegate().getBoolean("nativeStoreForceSync", false);
    }
	...
	public String getTitlePrefix() {
	 ...
	 public Boolean getSemanticWikiLinksEnabled()
	 {
