...
public class IwbStart
{
    ...
    /**
     * Name of the temporary file holding the number of restart attempts while installing the solution. 
     */
    private static final String RESTART_ATTEMPTS_FILE_NAME = "restartAttempts.tmp";
    
    /**
     * Maximal number of restarts to perform if the solution installation completes with the 
	...
	public static void initializeApp() throws Exception {
		System.setProperty("product.name", "Information Workbench");
		
		// initialize logging for iwbcom
		/* create logging dir if not exist */
		GenUtil.mkdirs(new File("logs"));
	...
	public static void initializeIWBWebapp(String j2eeWebPrefix) throws Exception {
        ...
        // tweak config: change the default start page of IWB only
        com.fluidops.config.Config.getConfig().setStartpageDefault("Help:Start");
	...
	public static void initializeWebapp(String j2eeWebPrefix) throws Exception
	{
        ...
        // load ontology terms (classes and properties)
        // which will be used for selection in the SPARQL query interface.
        SparqlServlet.initOntologyTermsSuggestions();
		...
		// register custom value resolvers
		ValueResolverUtil.initializeValueResolverExtensions();
	...
	public static boolean installSolutions() throws Exception {
        ...
        if(compositeResult.getInstallationStatus().isRestartRequired()) {
        	int restartAttempts = getPastRestartAttempts();
        	if(restartAttempts<MAX_RESTART_ATTEMPTS) {
        		installLogger.info("Trying to restart. Restarting attempt: " + (restartAttempts+1) + " out of " + MAX_RESTART_ATTEMPTS);
        		FileUtil.writeContentToFile(Integer.toString(restartAttempts+1), IWBFileUtil.getFileInWorkingDir(RESTART_ATTEMPTS_FILE_NAME).getAbsolutePath());
        		api().restartService();
        		return true;
        	} else {
        		installLogger.warn("Error: solution could not be installed in " + MAX_RESTART_ATTEMPTS + " restart attempts. Please resolve problems manually.");
        	}
        }
        GenUtil.delete(IWBFileUtil.getFileInWorkingDir(RESTART_ATTEMPTS_FILE_NAME));
	...
	private static int getPastRestartAttempts() {
		try {
			File restartStatusFile = IWBFileUtil.getFileInWorkingDir(RESTART_ATTEMPTS_FILE_NAME);
	        if(restartStatusFile.exists()) {
		        String sContent = GenUtil.readFile(restartStatusFile).trim();
		        
		        if(StringUtil.isNotNullNorEmpty(sContent)) {
		        	return Integer.parseInt(sContent);
		        }
	        }
		} catch(Exception e) {
			installLogger.warn("Could not read the number of past attempts: " + e.getMessage());
			installLogger.warn("Stopping the restart sequence.");
			installLogger.debug("Details: ", e);
