...
public class GraphWidget extends AbstractWidget<GraphWidget.Config>
{
	...
	
	{
		...
		@ParameterConfigDoc(
				desc = "The central resource (URI) of a neighborhood graph. Default: Current resource")
        public URI center;
	...
	public FComponent getComponent(final String id)
    {
		...
		conf.center = conf.center==null ? getDefaultCenter() : conf.center;
		useThumbnails = conf.thumbnails;
		conf.maxDepth = conf.maxDepth==null ? Integer.valueOf(4) : conf.maxDepth;
		conf.spreadFactor = conf.spreadFactor==null ? Integer.valueOf(10) : conf.spreadFactor;
		conf.infer = conf.infer!=null && conf.infer;	
		graphId=id;
		...
		switch(conf.graphType)
		{
			case forcedirectedgraph: 			
				...
				input = getRDFAsJsonArray(conf.center, rep, conf.maxDepth, conf.spreadFactor);
				break;
			case hypertree:
				method = "initHypertree";
				input = getRDFAsJsonTree(conf.center, rep, conf.maxDepth, conf.spreadFactor);
				break;
			case rgraph:
				method = "initRGraph";
				input = getRDFAsJsonTree(conf.center, rep, conf.maxDepth, conf.spreadFactor);
			...
			case ontologygraph:
				...
				break;
			default:
				throw new IllegalArgumentException("Graph type not supported: " + conf.graphType);
		}
		...
		if (input == null) {
			 ...
			 */
			return WidgetEmbeddingError.getNotificationLabel(id, NotificationType.NO_DATA, conf.noDataMessage);
		}
	...
	{
	      String cp = EndpointImpl.api().getRequestMapper().getContextPath();
		  return Lists.newArrayList(cp+"/ajax/JIT/jit-yc.js", cp+"/ajax/JIT/excanvas.js", cp+"/ajax/JIT/jit_resources.js");
	}
}
