 ...
 * </code>
 * 
 * 6. Auto suggestion of URIs for a text field. The autoSuggest query is evaluated only 
 * once when the user starts typing in the text input. The query results are filtered 
...
public class SearchWidget extends AbstractWidget<SearchWidget.Config> 
{
    ...
    public static class ParameterConfig
    {
    	...
    	@ParameterConfigDoc(desc = "List of values to be used as selection options (if componentType is DROPDOWN) or for auto suggestion (if componentType is TEXTFIELD). Can contain a list of pre-defined values or an operator $...$ which would select these values from the RDF repository when the page is loaded.", type = Type.LIST)
    	public List<Value> values;
    	
    	@ParameterConfigDoc(desc = "A SELECT query with one projection used for auto suggestion of values for text fields. Unlike in the 'values' parameter, this query is evaluated when typing in the input box.", type = Type.TEXTAREA)
			...
			throws MalformedQueryException, QueryEvaluationException
	{
		...
		for (ParameterConfig parameter : parameters)
		{
			...
			if(parameter.componentType==ComponentType.TEXTAREA) {
				if(parameter.values!=null)
					throw new RuntimeException("For a TEXTAREA component, the 'values' parameter cannot be specified.");
				if(StringUtil.isNotNullNorEmpty(parameter.autoSuggest))
					throw new RuntimeException("For a TEXTAREA component, the 'autoSuggest' parameter cannot be specified.");
			}
		
			final FComponent comp = parameter.componentType.create(Rand.getIncrementalFluidUUID(), listener, this.getParameterValueURI(parameter.parameterType));

			AutoSuggester suggester = null;
			if(parameter.values!=null) {
				suggester = AutoCompleteFactory.createFixedListAutoSuggester(parameter.values);
			}
			
			if (comp instanceof FValueDropdown)
			{
				if(suggester!=null)
					((FValueDropdown)comp).setSuggester(suggester);
				else
					throw new RuntimeException("For a DROPDOWN component, the 'values' parameter has to be specified.");
				
				if(StringUtil.isNotNullNorEmpty(parameter.autoSuggest)) {
					throw new RuntimeException("The parameter 'autoSuggest' is only applicable to TEXTFIELD parameters, for DROPDOWN boxes 'values' must be used.");
				}
			}
			else if (comp instanceof FValueHoldingTextInput)
			{
				FValueHoldingTextInput t = (FValueHoldingTextInput)comp;
				if(StringUtil.isNotNullNorEmpty(parameter.autoSuggest)) {
					if(suggester==null) {
						suggester = AutoCompleteFactory.createQuerySuggester(parameter.autoSuggest, pc.value);
					} else {
						throw new RuntimeException("Both 'autoSuggest' and 'values' parameters are specified for the parameter " + parameter.name+". Only one is allowed.");
					}
				}
				
				if(suggester!=null) {
					t.setSuggester(suggester);
				} 
			}
			grid.add(comp);
			...
			List<InputTuple> tuples, List<String> queryTargets)
	{
		try
		{
			...
			String url = EndpointImpl.api().getRequestMapper().getSearchUrlForValue(query, "SPARQL", queryTargets);		
			...
			throws ParseException
	{
		...
		{
			String input;
			Object returnValue = tuple.component.returnValues();
			if(returnValue instanceof Value) {
				Value inputValue = (Value)tuple.component.returnValues();
				input = inputValue.stringValue();
			} else if(returnValue!=null) {
				input = returnValue.toString();
			} else {
				input = "";
			}
	...
	private URI getParameterValueURI(ParameterType paramType) {
		switch(paramType) {
		case URI:
