...
public abstract class AbstractChartWidget<T extends WidgetChartConfig> extends
	...
	protected final FComponent getComponent(String id) {
		...
		// only serial charts types (Bar, Line, Radar and StockChart) can display multiple datasets

		boolean groupByDatasetVariable = Boolean.FALSE;
		
		if(config instanceof WidgetSerialChartConfig && ((WidgetSerialChartConfig)config).groupByDatasetVariable != null)
		{
		...
		try {
			...
			ChartWidgetUtil.extractValuesAndLabels(res, values, labels,
					config.input, config.output, queryDM, groupByDatasetVariable);
