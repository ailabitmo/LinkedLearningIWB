...
public class SearchResultWidget extends AbstractWidget<WidgetVoidConfig> {
		...
		while (res.hasNext()) {
			...
			for (String name : res.getBindingNames()) {
				...
				// and literals as labels
					...
					String value = HTMLSanitizer.sanitizeLinks(val.stringValue());
	...
	private void populateTableModel(FTableModel tm, final Clustering clustering)
	{
		...
		for (final Value subject : clustering.subjects)
		{
			// subject as link
			FComponent sub;
			if(spc.queryLanguage.equals("KEYWORD")) {
				sub = new FValueWithMatchHighlight(Rand.getIncrementalFluidUUID(), subject, new Highlight(HTMLSanitizer.sanitizeLinks(spc.query)));
			} else {
				sub = new FValue(Rand.getIncrementalFluidUUID(), subject);
			}
			...
			for (Pair<Value, Value> cluster : clustering.clusterMap.get(subject))
			{
				...
				// and literals as labels
					...
					String value = "<em>" + HTMLSanitizer.sanitizeLinks(cluster.fst.stringValue()) + ":</em>";
					pred = new FHtmlString(Rand.getIncrementalFluidUUID(), value, value);
				}
//				pred.addStyle("float", "right");
				
				String value = HTMLSanitizer.sanitizeLinks(cluster.snd.stringValue());
