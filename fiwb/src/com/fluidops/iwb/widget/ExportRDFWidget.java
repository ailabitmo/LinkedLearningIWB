 ...
 * @author ango
 */
public class ExportRDFWidget extends AbstractWidget<WidgetVoidConfig>
{
	...
	/**
	 * URL encodes the contexts before they can be sent to the sparql servlet
	 * @param selectedContexts
	 * @return 
	 */
	protected List<String> urlEncode(List<URI> selectedContexts) 
	{
		List<String> encodedContexts = new ArrayList<String>();
		
		for(URI context : selectedContexts)
		{	
			//default context
			if(context == null)
				encodedContexts.add(null);
			else
				encodedContexts.add(StringUtil.urlEncode(context.toString()));
		}
		return encodedContexts;
	}
