...
import com.fluidops.ajax.components.FUpload.UploadType;
...
public class CMS extends FContainer
{
	...
	private FUpload createUploadComponent(final FComboBox collectorCombo, final FSelectableTable<File> filesTable) {
		...
		upload.setUploadType(UploadType.UploadOnButtonClick);
		upload.setShowProgress(true);
		return upload;
	}
				...
				try
				{
					...
					if ( files != null )
						for ( File f : files )
						{
							...
							addRow( new Object[] {
									new HtmlString( EndpointImpl.api().getRequestMapper().getAHrefFromValue( f.getURI(), false, true, null) ),
									mime == null ? null : mime.fst+"/"+mime.snd, 
									new Date(f.lastModified()), 
									StringUtil.displaySizeInBytes(f.length()), 
									new HtmlString( "<a href='"+IWBCmsUtil.getAccessUrl(f)+"'>"+f.getName()+"</a>" ),
