...
public class ReadDataManagerImpl implements ReadDataManager
{
    ...
    // cache
    private static ConcurrentMap<Repository,ReadDataManagerImpl> dms = 
        new ConcurrentHashMap<Repository,ReadDataManagerImpl>(16, 0.75f, 2);
	...
	public String getLabel(Value val)
	{
	    ...
	    lblStr = getValueForProperties(valAsUri, labelProperties, Config.getConfig().getPreferredLanguage());
        ...
        while (res.hasNext())
        {
            ...
            for (int i=0;i<outputs.length;i++)
            {
                ...
                if (aggType == AggregationType.NONE)
                {
                    ...
                    if (b!=null)
                        curValues.set(i, (b.getValue() == null) ? null : ((Literal)b.getValue()).doubleValue());
                }
    ...
    /**
     * Gets RDFFormat string from the given string. The string
     * can either be the MIME type of a valid RDF format 
     * (e.g. application/rdf+xml) or the name of any {@link RDFFormat}
     * value (e.g. RDF/XML, N-Triples, Turtle, N3, TriG)
     * 
     * This method returns null if the given RDFFormat cannot
     * be parsed.
     * 
     */
    public static RDFFormat parseRdfFormat(String format)
    {
    	// test for mime type first, then use the simple name
    	RDFFormat contentType = RDFFormat.forMIMEType(format);
    	if (contentType!=null)
    		return contentType;
    	
    	// test the simple name next
    	...
    	// support various legacy notations (which were documented in the CLI)
	...
	public static void shutdown()
    {
	    ...
	    dms = new ConcurrentHashMap<Repository,ReadDataManagerImpl>(16, 0.75f, 2);
    }
