...
public class CommunicationServiceImpl implements CommunicationService, Serializable
{
    ...
    @Override
    public boolean validateOntology() throws Exception
    {
    	Collection<Class<?>> annotatedClasses = OWLAnnotatedClassRegistry.getInstance().getRegisteredClasses();
    	ReadWriteDataManager dm = 
            ReadWriteDataManagerImpl.openDataManager(Global.repository);

        try {
	        // We need to delete potential old ontologies extracted via the annotation framework
	        dm.deleteExpiredContextsOfSource(ontologyContext, null, null, null);
	
	        // iterate over the classes and validate them
	        Set<Statement> stmts = Sets.newHashSet();
	        for (Class<?> clazz : annotatedClasses)
	        {
	            ...
	            catch (NoAnnationException e)
	            {
	            	...
	            	logger.debug(String.format("The class '%1$s' is registered in the OWLAnnotatedClassRegistry but it is not annotated", clazz));
	            }
	            catch (Exception e)
	            {
	        ...
	        List<String> errors = getOntologyMisalignments(dm, stmts);
	...
	/**
	 * Checks that all the statements provided as {@code statements} have equivalents inside the {@link ReadDataManager} {@code dm}.
	 * 
	 * @param dm The {@link ReadDataManager} that contains the triples to be checked against those included in {@code statements}.
	 * @param statements The triples that must all be present inside {@code dm}
	 * @return A list, possibly empty in case of no misalignment, of errors.
	 */
	public static List<String> getOntologyMisalignments(ReadDataManager dm,
			Iterable<Statement> statements) {
		List<String> errors = Lists.newArrayList();
		for(Statement s : statements)
		{
			if(s.getPredicate().equals(RDF.TYPE) && (!dm.hasStatement(s.getSubject(), s.getPredicate(), s.getObject(), false)))
				errors.add(s.getSubject() + " is not properly declared in the ontology");
		}
		return errors;
	}
     ...
     * updating data in the RDF database.
     * 
     * @author msc
     *
     */
