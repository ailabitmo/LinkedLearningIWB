...
/**
 * retrieves dom element by id
 * note: using jquery for dom retrieval would return wrapper object
 * jquery $ (id) - id needs to be escaped (e.g. dots)
 * therefore using jquery here would break a lot as we use random IDs in some places
 * @param id, e.g. 'myId'
 * @returns the dom element matched by id
 */
...
function debug( message )
{
	...
	var debugWnd = getDomElementById('debugWnd');
	if(debugWnd == null)
	{
		...
		debugWnd = getDomElementById('debugWnd');
	}
...
{
	if ( getDomElementById('progress')==null )
	{
...
function asyncStatusInProgress()
{
	...
	if(currentEventType !== 13)
	{
		...
		getDomElementById('progressImg').src = asyncStatusProgressIcon;
		getDomElementById('progressImg').alt = '';
		getDomElementById('progress').style.display = 'block';
		getDomElementById('progressImg').title = '';
...
function asyncStatusSuccess()
{
	...
	getDomElementById('progress').style.display = 'none';
	getDomElementById('progressImg').alt = '';
	getDomElementById('progressImg').title = '';
...
function asyncStatusError( error )
{
	...
	getDomElementById('progressImg').src = asyncStatusErrorIcon;
	getDomElementById('progressImg').width = '32';
	getDomElementById('progressImg').alt = error;
	getDomElementById('progressImg').title = error;
...
function asyncUpdatePeriodical(id, type, arg, callback, timeout)
{
	...
	if(getDomElementById(id))
	{
		...
		//updateTime();
		getDomElementById(id).timeout = setTimeout(function(){asyncUpdatePeriodical(id, type, arg, callback, timeout);}, timeout);
	}
	else
	{
...
//if id is not there anymore recursion will be stopped automatically @see #asyncUpdatePeriodical
	if(getDomElementById(id))
	{
		var timeoutVar = getDomElementById(id).timeout;
...
{
	var node = getDomElementById(id+".input");
	var mInfo = global[ id ];
	if ( mInfo==null )
...
function enableInputMonitor( id )
{
	...
	getDomElementById(id+"_dropdown").style.visibility="";
}
...
function disableInputMonitor( id )
{
	...
	if ( mInfo!=null )
	{
		...
		setTimeout( "getDomElementById('"+id+"_dropdown').style.visibility='hidden';", 1000);		
	}	
}

function flTabPane_show( id, pane )
{
	var element = getDomElementById(id+'.hidden');
	var i;
	for (i=0; ; i++)
	{
		var elementToAppend = getDomElementById(id+'.'+i);
	...
	getDomElementById(id+'.content').appendChild(getDomElementById(id+'.'+pane));
	//looks for the first ul
	var u = getDomElementById(id).getElementsByTagName('ul')[0];
...
{
	getDomElementById(id).style.display='none';
}
...
{
	return getDomElementById(obj.parentNode.id + '_dropdown');
}
...
function setItemValue( inputid, items, itemid )
{
	...
	for(i = 0; i < items.length; i++)
	{
		if(i == itemid)
		{
			getDomElementById(inputid).value = items[i].value;
			return;
		}
	}
}
...
{
	if (getDomElementById(id) == undefined && !debugEnabled)
		return;
	
	cbAddEventListener(getDomElementById(id), eventName.toLowerCase(),
			function() 
			{
				var el = getDomElementById(id);
				var i;
		...
		exclusivityCancel)
{
	...
	getDomElementById(id)['reported' + eventName] = function() {
		var el = getDomElementById(id);
...
function isDropDownVisible( textInputAnchorId ){
	 var dropdown = getDomElementById(textInputAnchorId + '_dropdown');
...
function textInputOnFocus( textInputAnchorId )
{
	...
	var input = getDomElementById(textInputAnchorId + '.input');
	var dropdown = getDomElementById(textInputAnchorId + '_dropdown');
...
function setSizeSuggList(dropdown){
	...
	dropdown = getDomElementById(dropdown);
...
function setDropdownVisibilityId( dropdown, visibleFlag, timeout) {
	if(getDomElementById(dropdown)==null){
		return;
	}
	setDropdownVisibility(getDomElementById(dropdown), visibleFlag, timeout);
...
function sendSelectionToInput( inputElId, newValue, hideDropdown ) {
	var inputEl  = getDomElementById(inputElId);
...
function keypressAutocompleteHandler1( event, id )
{
	...
	case 13: 	// ENTER
				...
				if (selItem == -1) 
					catchEventId(id,12,getDomElementById(id+'.input').value);
 ...
 function keyreleaseAutocompleteHandler1( event, id )
 {
	debugln('keyrelease1 '+event.keyCode);
 	var sender = getEventSender(event);
 	var dropdown = getDropDownControlOf( sender );
 	var arg = getDomElementById(id+'.input').value;
...
/*******************************************************************************
 * 
 * Necessary functions for FTabPane2Lazy
 * if #enableClientSideTabCaching is true
 * 
 ******************************************************************************/

/**
 * puts a tab element inside the dom (relative to its tabPane) 
 * in order to support client side caching
 * @param tabPaneLazyId - tabpane2lazy id
 * @param hiddenElementId - id of the hidden cache div
 * @param tabElementId - id of the tab element
 */
function storeTabContentInHtmlDoc(tabPaneLazyId, hiddenElementId, tabElementId)
{
	//get the hidden element
	var hiddenElement = getDomElementById(hiddenElementId);
	//if the hidden element is not there, create it
	if(!hiddenElement)
	{
		debugln("creating cache store "+hiddenElementId);
		hiddenElement = document.createElement("div");
		hiddenElement.id = hiddenElementId;
		hiddenElement.style.display = "none";
		//hidden Element will be stored relative to the tabPane2Lazy id 
		// --> to support multiple tabPanes on same page
		
		//append hiddenElement to parentElement, because tabpane content will be reset on #populateView
		getDomElementById(tabPaneLazyId).parentNode.appendChild(hiddenElement);
	}
	debugln("storeElementInCache\t"+hiddenElementId+" << "+tabElementId);
	//add the tabElement to the hiddenElement 
	var eleToStore = getDomElementById(tabElementId);
	
	//clear cache
	hiddenElement.innerHTML = "";
	//place new element in there
	hiddenElement.appendChild(eleToStore);
}

/**
 * retrieves a tabElement from the cache and puts it back into the tab (inside tabPane2Lazy)
 * @param tabElementId - id of the tab element
 * @param hiddenElementId - id of the hidden element which stores the tabs
 */
function switchTabContentFromHtmlDoc(tabElementId, hiddenElementId)
{
	var hiddenElement = getDomElementById(hiddenElementId);
	var tabElement = getDomElementById(tabElementId);
	var tabParent = tabElement.parentNode;
	//reset parentNode
	tabParent.innerHTML = "";
	if(!hiddenElement)
	{
		debugln("Failed to switch tab cotnent from cache element ("+hiddenElementId+")!");
		tabParent.innerHTML = "Failed to get tab content from cache.";
		return;
	}
	var eleToExpose = hiddenElement.childNodes[0];
	debugln("replaceTabWithElementFromCache\t"+hiddenElementId+" > "+eleToExpose.id);
	
	//replace current tab innerHTML with element from cache
	tabParent.appendChild(eleToExpose);
}


// TODO: use http://jqueryui.com/slider/ instead
...
/**
 * TODO: use http://jqueryui.com/slider/
...
function setBoxValue(val, granularity, sliderid)
{
	...
	var slider = getDomElementById(sliderid);
...
{
 	var divs = getDomElementById(domid).getElementsByTagName('div');
...
function updateGauge(gaugeId, position)
{
	...
	var gaugel = getDomElementById(gaugeId);
...
function createGauge(canvId, start, end, position, options )
{
	...
	if (typeof(G_vmlCanvasManager) != 'undefined')
		this.canvas = G_vmlCanvasManager.initElement(getDomElementById(this.canvId));
	else
		this.canvas = getDomElementById(this.canvId);
...
function debugTableResize( message ) {
	if (debugMode) {
		var _debug = getDomElementById('debug-div');
		if (_debug)
...
{
	var el = (invoker.id.indexOf('Input')==-1)?(getDomElementById( invoker.id + 'Input')):invoker;
	var value = ExtractNumber( el.value);
	if (value<=0) {
		invoker.value = getDomElementById(invoker.id+'Hidden').value;
...
function resizeTableCells( diff, mousePosX ) {
	...
	getDomElementById('splitterDiv').style.left = mousePosX + 'px';
}
/**
 * 
 * @param event
...
function tableResize_OnMouseUp(e)
{
    if (_splitter != null)
    {
        ...
        getDomElementById('splitterDiv').style.display = 'none';
...
function mouseDownOnSplitter( event, splitter ) {
    ...
    var splitterDiv = getDomElementById('splitterDiv');
...
function toggleDropdown(el, style) {
	getDomElementById(el).style.display=style;
}
...
function tt2show(el,parid,inner,wholewidth,attachtoparent) {
	...
	var tt2 = getDomElementById('tooltip2');
	
	tt2.innerHTML = inner;
	
	var dtt2 = tt2getDimensions(tt2);
	var dpid = tt2getDimensions(getDomElementById(parid));
	var del = new tt2dimension();
	
	var tempel = getDomElementById(el);
...
function tt2add() {
	if(getDomElementById('tooltip2') == null) {
...
function fformSetValid(id,b,fc) {
	var clazz = getDomElementById(id).className;
	if(fc)
		clazz = getDomElementById(id).firstChild.className;
	...
	if(fc)
		getDomElementById(id).firstChild.className = clazz;
	else
		getDomElementById(id).className = clazz;
}
 
 function getMultipleComboBox(select){
	var str= new Array();
	select = getDomElementById(select.id);
 ...
 function selectMultipleValuesInCombobox(id,opts){
	 var select = getDomElementById(id);
	 var optselected = 0;
	 for ( var i = 0; i < select.childNodes.length; i++) {
 ...
 function highlightFormRows(id, labelToFind, className)
 {
	 var labels = $('#'+id+' > fieldset > DIV > label');
	 $(labels).each(function(){
		 if(this.firstChild 
				 && this.firstChild.nodeValue 
				 && this.firstChild.nodeValue == labelToFind)
		 {
			 $(this).addClass(className);
		 }
	 });
	 $('#'+id+' > fieldset > p.annotation').addClass(className);
 }
 
 /**
  * clears all formRows within the form identified via id (includes nested forms)
  * @param id - dom id of the form
  * @param className - classname which will be removed (e.g. 'warning')
  */
 function clearHighlightedFormRows(id, className)
 {
	 $('#'+id+' fieldset > DIV > label').removeClass(className);
	 $('#'+id+' fieldset > p.annotation').removeClass(className);
 }
...
function toggleContent(contentid,buttonid,buttonClassShow,buttonClassHide){
	 var show;
	 if(getDomElementById(contentid).style.display == 'none'){
		 getDomElementById(contentid).style.display = 'block';
		 show = true;
	 }else{
		 getDomElementById(contentid).style.display = 'none';
		 show = false;
	 }
	 
	 if(show){
		 getDomElementById(buttonid).className = buttonClassHide;
		 getDomElementById(buttonid).setAttribute("class",buttonClassHide);
	 }else{
		 getDomElementById(buttonid).className = buttonClassShow;
		 getDomElementById(buttonid).setAttribute("class",buttonClassShow);
	 }
}
 ...
 */
function show(id){
	getDomElementById(id).style.display = '';
}
...
function slide(comp, currentHeight, totalHeight, px, direction, initialHeight, initialOverflow){
 ...
 function toggleDisplay(id){
	getDomElementById(id).style.display=(getDomElementById(id).style.display=='none')?'':'none';
...
function hidePopup(){
	...
	getDomElementById('FPopupWindow').className = 'hidden';
...
function centerPopUp(){
	...
	var elems = getElementsByClassName('flDialog'); // get all elements with class-name 'flDialog' because some java-classes use new FPopupWindow() instead of getPopupWindow()
	...
	{
		//should not happen
		debugln("Failed to center dialog. No elements with class flDialog found")
	}
}
...
/****************************************************************************
 * 
 * File upload function with optional progress display
 * Progress display works in FF, Opera and Safari, 
 * no progress bar is displayed in IE
 * @param file input container id 
 * @param progress display container id
 * @param progress state container id
 * @param boolean showProgress - defines whether the progress bar should be displayed or not
 ****************************************************************************/

function uploadFile(inputID, progressBarID, progressStateID, showProgress) {

    var fileInput = document.getElementById(inputID);
    
    // progress display doesn't work in IE, so just send the data
    if(!showProgress || checkBrowserName('MSIE') )
        { fileInput.parentNode.submit(); return;}
    ...
    if (xhr.upload) {
            ...
            var progressPercent = document.getElementById(progressStateID);
            progressPercent.style.innerHTML= '';
            ...
            xhr.addEventListener('error', function(e) {
                progressBar.innerHTML = "<p>"+e.responseText+"</p>";
            }, false);
            
        }
            // start upload
        xhr.open("POST", fileInput.parentNode.action, true);

        var data = new FormData(fileInput.parentNode);
        xhr.send(data);
 } 
